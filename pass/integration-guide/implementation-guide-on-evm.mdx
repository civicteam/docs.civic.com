...
While the [React library](/pass/integration-guide/implementation-guide-on-evm#install-the-react-component) is the easiest way to integrate Civic Pass on a frontend, there are many reasons why you may need to check the pass state on your backend. Here are a few ways to do this.
...
The Gatekeeper Context will communicate a new presentation request ID when the user reaches the end of the data collection flow. See [here](/pass/integration-guide/implementation-guide-on-evm#install-the-react-component) for more detail on the use of `GatewayProvider` and `useGateway`.
