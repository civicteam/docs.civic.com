...
- **Credential access is restricted** — only credentials [explicitly assigned](/civic/reference/authorizations#profile-assignment) to the profile are available. Unassigned credentials are not resolved, even if they would otherwise match.
...
| API key authentication | Always locked (with `END_USER` role) |
