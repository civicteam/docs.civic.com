---
title: Vercel AI SDK
description: Build AI applications using Civic with the Vercel AI SDK (v5)
icon: "triangle"
public: true
---
...
Build AI applications with Civic and Vercel AI SDK v5. Your AI agents can access external tools and services through MCP (Model Context Protocol) - interact with GitHub, Slack, Dropbox, and more.
...
Want to integrate Civic into an existing Next.js project? Follow the guide below.
...
export const getCivicTools = async () => {
  ...
  if (!accessToken) {
    // Return empty tools if no access token (Civic is optional)
    return {};
  }
