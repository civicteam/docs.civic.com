...
Integrate Civic with OpenAI Agents SDK using the `hostedMcpTool()` approach. This is the **simplest** way to use Civic with OpenAI - no manual tool looping or conversion code required.

<Note>
**Updated for OpenAI Agents SDK**: This guide uses the new hosted MCP tool feature. For the older function calling approach, see [OpenAI SDK (Node.js)](/civic/recipes/openai-sdk).
</Note>
...
2. Civic account - [Create account](https://app.civic.com)
3. Connect services at [app.civic.com](https://app.civic.com) (GitHub, Slack, etc.)
...
<Note>
**Why do I need a token?** OpenAI Agent Builder requires an access token upfront because it runs server-side without browser access for OAuth flows. [Learn more about Civic tokens](/civic/concepts/tokens).
</Note>
...
async function createCivicAgent() {
  ...
  return new Agent({
    ...
    model: 'gpt-4o-mini',
    instructions: 'You are a helpful assistant with access to external tools through Civic. Use the available tools to help users with their tasks.',
    tools: [
      hostedMcpTool({
        serverLabel: 'civic',
...
async function main() {
  const agent = await createCivicAgent();
...
- Discovers available tools from Civic
...
<AccordionGroup>
  ...
  <Accordion title="Civic Endpoint">
    ```
...
<Steps>
  ...
  <Step title="Connect More Services">
    Visit [app.civic.com](https://app.civic.com) to connect additional tools
  </Step>
