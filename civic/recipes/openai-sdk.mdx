---
title: OpenAI SDK
description: Use OpenAI with Civic tools (function calling) without the Vercel AI SDK
icon: "robot"
public: true
---
...
This recipe shows how to wire Civic MCP tools to the official OpenAI Node SDK using function calling.
...
**Want a simpler approach?** Check out the [OpenAI Agents SDK recipe](/civic/recipes/openai-agents) which has native MCP support via `hostedMcpTool()` - no manual tool looping required!
</Note>
...
async function createMCP() {
  ...
  const transport = new StreamableHTTPClientTransport('https://app.civic.com/hub/mcp', {
