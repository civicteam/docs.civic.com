...
When a profile is [locked](/civic/quickstart/clients/agents#profile-locking) — for example, when using a profile-scoped token — only credentials explicitly **assigned** to the profile are available. Unassigned credentials are not resolved, even if they would otherwise match.
