...
<Info>
  **Creating vs using a wallet**
  ...
  Civic Auth automatically provisions a wallet for new users via `createWallet()` or the `useAutoConnect()` hook. The table above describes how to *use* an already-provisioned wallet in your UI — no additional "create" call is needed in the components that read wallet data.
</Info>
