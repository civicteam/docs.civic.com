...
description: >-
  How an MCP client can handle authentication automatically using the
  civic:rest-auth capability
---
...
Civic advertises the same capability in its server capabilities. When both sides declare it, Civic switches to the `rest-auth` strategy for this session.
...
```json
{
  "content": [
    ...
    },
    {
      "type": "resource",
      "resource": {
        "uri": "https://app.civic.com/jobs/job-uuid-here",
        "mimeType": "text/plain",
        "text": "job-uuid-here",
    ...
    },
    {
      "type": "resource",
      "resource": {
        "uri": "https://app.civic.com/user/jobs/550e8400-.../status",
        "mimeType": "text/plain",
        "text": "https://app.civic.com/user/jobs/550e8400-.../status",
...
sequenceDiagram
    ...
    Client->>Civic: initialize (civic:rest-auth)
    Civic-->>Client: capabilities (civic:rest-auth)

    Client->>Civic: tools/call (e.g. notion_search)
    Civic-->>Client: auth response (3 items: text, jobId, polling)

    Note over Client: Intercept response

    Client->>Civic: GET pollingEndpoint
    Civic-->>Client: AUTH_REQUIRED + authUrl

    Note over Client: Open authUrl in browser

    Client->>Civic: HEAD pollingEndpoint
    Civic-->>Client: 202 (waiting)

    Note over Client,Civic: User completes auth in browser

    Client->>Civic: HEAD pollingEndpoint
    Civic-->>Client: 200 (approved)

    Client->>Civic: tools/call continue_job
    Note over Civic: Execute original tool
    Civic-->>Client: tool result
...
- The polling endpoint **never** returns the actual token. It only returns job status (`PENDING`, `AUTH_REQUIRED`, `APPROVED`, `ERROR`) and the `authUrl` when applicable. The token is only exchanged server-side within Civic when `continue_job` is called.
...
- The `authUrl` points to Civic, which handles the OAuth flow securely (consent screen, PKCE, token exchange).
