...
title: React
description: >-
  Integrate Civic Auth into your React application with ease, just wrap your app
  with the Civic Auth provider and add your Client ID (provided after you [sign
  up](https://auth.civic.com)).
...
<Frame caption="The user button">
  <img src="/images/auth/integration/image-1.png" className="image-50 image-rounded" />
</Frame>
...
When sending the access token from your frontend to your backend for verification, you can access it from the `userContext` object:

```typescript
const { accessToken, idToken } = useUser();
// Send accessToken and/or idToken to your backend for verification
```

To verify the token signature on your backend, use the `@civic/auth-verify` library which automatically queries the Civic JWKS endpoint to fetch the public key:
...
// Verify the access token and id tokens
await verify(accessToken);
const payload = await verify(idToken);
console.log(payload); // Contains user information
```

For complete backend verification examples and details, see the [`@civic/auth-verify` documentation](/libraries/auth-verify).
</Note>
