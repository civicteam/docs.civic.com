...
description: >-
  Understanding Civic Auth's flexible OAuth 2.0 authentication methods for
  maximum security across all application types.
...
<Note>
For full implementation examples with specific frameworks, see our [integration guides](/integration/react). The guides show the PKCE-only approach by default, but you can adapt the `config` object using the parameters above for other flows.
</Note>
