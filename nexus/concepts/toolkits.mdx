...
- **Create toolkits with as few tools as possible.** Use broad toolkits to help with configuration and data gathering, then once you know what you want, bake these into a new toolkit, and filter out all tools you don't need.
...
- **Parameter presetting and tool filters can be used as guardrails,** preventing LLMs from accessing resources or performing actions that they are not permitted to. For more fine-grained guardrails, ask the LLM which guardrails are available to it. *Do not* use description fields as guardrails - unlike tool filters and parameter presets, these are guidelines only and LLMs are not beholden to them.
