...
Quickly connect Civic to Claude Code using the MCP CLI workflow:

<Steps>
  <Step title="Register the Civic MCP server">
  ...
  <Step title="Launch Claude Code">
    Start the Claude CLI by running:
  ...
  <Step title="Connect and sign in">
    In the Claude prompt, type `/mcp`, choose the `civic` server, and follow the Civic login flow to authorize access.
  </Step>
</Steps>
...
<Frame>
  <img
    ...
    alt="Connecting Civic within Claude Code"
