...
The IMAP Email server provides read and management access to any IMAP-compatible email provider including Gmail, Outlook, Yahoo, and self-hosted servers. It supports searching, reading, organizing, and managing emails across folders.

<Note>
**Read-only by design:** This server cannot send or compose emails. It is designed for reading and managing existing emails only. For sending email via Gmail, use the [Google Gmail](/nexus/reference/servers/gmail) server. For Outlook/Microsoft 365, use the [Microsoft 365 Mail](/nexus/reference/servers/ms365-mail) server.
</Note>
...
<Steps>
  ...
  <Step title="Configure Credentials">
    ...
    - **IMAP host** — your email provider's IMAP server (e.g. `imap.gmail.com`, `imap.mail.yahoo.com`, `outlook.office365.com`)
...
<Warning>
**App-specific passwords required for 2FA accounts:**
- **Gmail**: Go to Google Account → Security → 2-Step Verification → App passwords
- **Yahoo**: Go to Account Info → Account Security → Generate app password
- **Outlook/Microsoft 365**: Consider using the [Microsoft 365 Mail](/nexus/reference/servers/ms365-mail) server instead for OAuth-based access
</Warning>
