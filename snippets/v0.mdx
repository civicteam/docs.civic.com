...
**CRITICAL**: You MUST have the official documentation before proceeding. Try to fetch it, or ask the user to provide it:

**Option 1 - Try curl (if available):**
```bash
curl https://docs.civic.com/integration/nextjs.md
```

**Option 2 - If curl fails or is not available:**
Ask the user: "Please provide the content from https://docs.civic.com/integration/nextjs or paste the documentation content here."
...
**If documentation is not available and user cannot provide it, use this essential structure:**
The integration requires these 4 core steps from the official docs:
1. **Add Civic Auth Plugin** in next.config.ts with createCivicAuthPlugin
2. **Create API Route** at src/app/api/auth/[...civicauth]/route.ts  
3. **Add Middleware** at src/middleware.ts with authMiddleware
4. **Frontend Integration** with CivicAuthProvider wrapper
5. **Add UserButton** component from @civic/auth/react
...
- If curl is not available or still failing, ask user: "Please provide the content from https://docs.civic.com/integration/nextjs"
...
If the user provides the documentation content (instead of curl), you should:
