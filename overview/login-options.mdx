---
title: "Login Options"
description: "The login options you can set up for users to log in to your app."
icon: arrow-right-to-bracket
public: true
---




You can choose different Login Options for your App using the [Civic Auth Dashboard](https://auth.civic.com/dashboard). You can choose which providers you'd like to support, and the order you'd like to present them to your users.

Just click on a provider icon to enable or disable it, and drag the option above or below existing options to change the ordering. As soon as you make an update, you'll see a preview of what your users will see.

<Frame>
    <img src="/images/login-options-dashboard-sso-email.png"/>
</Frame>

## Social Providers (OAuth)

We currently support the following social providers:
- Google
- Discord
- Github
- Facebook
- X
- Apple
- Microsoft (coming soon)

Don't see your preferred provider? [Let us know](https://join.slack.com/t/civic-developers/shared_invite/zt-37tv9fyo7-aDT43mUjOFQwdQFmfZLTRw) and we'll see what we can do!

## Email

Users login by proving ownership of their email address by entering an OTP (One-Time PIN) sent to their email.

## Account linking

When a user logs in using an email address, once the OTP code is verified, we check if the email has already been used with a Google or Apple account.

If the email address associated with one of these providers matches the email address used to login, we automatically link the accounts. This means that the returned user id will be the same for both login methods, and the user's profile information will be merged, i.e. their name, profile picture, and any other relevant information.

## Passkey

Passkeys offer a secure and easy replacement for passwords. Instead of a password, users sign in by approving the login request through a simple, secure action, such as:
- A biometric scan or PIN on their device (phone or computer)
- Tapping a physical hardware key (e.g., YubiKey)
- Authenticating via a third-party service like a password manager (e.g., Bitwarden)

To enable the creation and usage of passkeys for your app, go to Application Options and toggle the "Enable Passkeys" setting. You can also decide how often you'd like your users to be prompted to create a passkey, based either on the number of logins or the time since their last login.

### End User Experience

When passkeys are enabled, users will be prompted to create a passkey during the login process, after they have logged in with one of the existing login methods like email or social login. The next time they visit your site they'll be prompted to use the passkey they created.

Users will always have the option of canceling a passkey login and instead using a different login method.

<img
src="/images/Passkey.png"
align="center"
className="image-60"
alt="Passkey UI"
title="Passkey UI"
/>

<Note>Note that due to browser iframe security restrictions, users will go through the full-page authentication flow, rather than authenticating in an iframe.</Note>

*Key Benefits:*
- Enhanced Security: Passkeys are built on the WebAuthN standard, making them strongly resistant to phishing and server-side data breaches.
- Effortless User Experience: Users no longer need to create or remember complex passwords, leading to faster, more convenient sign-ins and fewer account lockouts.
- Cross-Device Syncing: Passkeys can be synced across a user's devices through their platform account (like an Apple ID or Google Account), enabling a seamless login experience on any of their devices.

To learn more about the underlying cryptographic standard, please refer to the official [WebAuthN documentation](https://www.w3.org/TR/webauthn/).
