...
If you're using a backend server (Express, Fastify, Hono, etc.) for authentication, you can configure the React components to work with your backend using `loginUrl` instead of `clientId`:
...
<Info>
When using `loginUrl`, your backend server handles the OAuth flow and session management. See our backend integration guides for [Express](/integration/nodejs/express), [Fastify](/integration/nodejs/fastify), or [Hono](/integration/nodejs/hono).
</Info>
