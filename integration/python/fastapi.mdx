...
## 7. Custom Session Storage

By default, FastAPI integration uses secure cookies. You can customize the storage:
...
class CustomCookieStorage(CookieStorage):
    def __init__(self, request: Request, response: Response):
        super().__init__(request, response, settings={
            "secure": True,  # Use HTTPS in production
            "httponly": True,
            "samesite": "lax"
        })

# Use custom storage in your routes
@app.get("/auth/callback")
async def callback(request: Request, response: Response, code: str, state: str):
    storage = CustomCookieStorage(request, response)
