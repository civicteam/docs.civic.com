...
def dashboard(request):
    ...
    return HttpResponse(f"""
        ...
        <img src="{user.picture}" alt="Profile picture" style="width: 50px; height: 50px; border-radius: 50%;">
...
def protected_view(request):
    user = request.civic_user
    return HttpResponse(f"Hello {user.name}, this is protected!")
```
...
<body>
    {% if civic_user %}
        <h1>Welcome, {{ civic_user.name }}!</h1>
        <img src="{{ civic_user.picture }}" alt="Profile picture">
        <p>Email: {{ civic_user.email }}</p>
