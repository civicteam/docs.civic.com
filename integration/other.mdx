...
<Info>
  **Need help choosing the right flow?** See our [Authentication Flows guide](/overview/authentication-flows) for detailed comparison of PKCE vs client secret approaches, including when to use each and how to request client secret support.
</Info>

<Warning>
  The examples below demonstrate the **PKCE flow** (recommended for most applications). For client secret authentication, contact us at [support@civic.com](mailto:support@civic.com) or see our [Authentication Flows guide](/overview/authentication-flows).
</Warning>
