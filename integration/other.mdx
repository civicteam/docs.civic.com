...
At its core, Civic Auth is an [OAuth 2.0 auth server](https://oauth.net/2/). It supports the [authorization code](https://oauth.net/2/grant-types/authorization-code/) grant type with both [PKCE](https://oauth.net/2/pkce/) and traditional client secret authentication.

<Info>
  **Need help choosing the right authentication method?** See our [Authentication Flows guide](/overview/authentication-flows) for detailed comparison of PKCE-only, client secret, and hybrid approaches, including when to use each for maximum security.
</Info>

<Warning>
  The examples below demonstrate the **PKCE-only method** (suitable for public clients). For client secret or hybrid PKCE + client secret authentication, contact us at [support@civic.com](mailto:support@civic.com) or see our [Authentication Flows guide](/overview/authentication-flows).
</Warning>
