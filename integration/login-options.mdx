...
Don't see your preferred provider? [Let us know](https://civickey.typeform.com/req-custom) and we'll see what we can do!
...
When a user logs in using an email address, once the OTP code is verified, we check if the email has already been used in a social provider that verifies email addresses (currently Google, Apple and Microsoft).
