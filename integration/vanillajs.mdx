...
If you prefer to handle authentication through your backend (useful for server-side session management), you can integrate with an Express backend instead of using the client-side approach above.

With this approach, your vanilla JavaScript frontend makes HTTP calls to your backend's auth endpoints:

```javascript
// Instead of using @civic/auth/vanillajs, use fetch to your backend
...
See the [Express integration guide](/express#frontend-integration-vanilla-javascript) for complete setup instructions.
