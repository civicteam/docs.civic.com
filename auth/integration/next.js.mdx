...
The defaults should work out of the box for most customers, but if you want to configure your app, see [below](/auth/integration/next.js#advanced-configuration) for details.
...
<Info>
  Unlike the pure [React](/auth/integration/react) integration, you do *not* have to add your client ID again here!

  Make sure to create the [Civic Auth API route](/auth/integration/next.js#id-2.-create-the-civic-auth-api-route), as it serves the essential PKCE code challenge.
</Info>
...
The Next.js integration can use all the components described in the [React integration page](/auth/integration/react), such as the `UserButton` , for showing a Sign-In button and displaying the username:
...
See the [React Usage page](/auth/integration/react) for more details.
...
<Info>
  The `name` property is used as an example here, check out the [React Usage page](/auth/integration/react) to see the entire basic user object structure.
</Info>
...
| `include`         | No       | `["/*"]`               | `["/admin/*", "/api/admin/*"]` | An array of path [globs](https://man7.org/linux/man-pages/man7/glob.7.html) that require a user to be logged-in to access. If not set, will include all paths matched by your Next.js [middleware](/auth/integration/next.js#middleware). |
| `exclude`         | No       | -                      | `["public/home"]`            | An array of path [globs](https://man7.org/linux/man-pages/man7/glob.7.html) that are excluded from Civic Auth [middleware](/auth/integration/next.js#middleware). In some cases, it might be easier and safer to specify exceptions rather than keep an inclusion list up to date.|
